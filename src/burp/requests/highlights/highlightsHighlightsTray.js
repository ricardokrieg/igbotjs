...
const highlightsHighlightsTray = async (client, userId) => {
  const debug = _debug('bot:highlightsHighlightsTray');
  ...
  const response = await client.send({ url: `/api/v1/highlights/${userId}/highlights_tray/`, qs });
  debug(response);

  return response;
};

module.exports = highlightsHighlightsTray;
